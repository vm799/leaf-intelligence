...
const fda483Schema = new mongoose.Schema({
  ...
  Download: String,
  Record_ID: String,
  // New field for raw text
  rawText: String,
  textExtractedDate: Date,
  textExtractionError: String,
  textExtractionAttempts: { type: Number, default: 0 }
});

const FDA483 = mongoose.model('fda_database.483s', fda483Schema);
...
async function downloadPDF(url, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        console.log('Attempting download with Puppeteer...');
...
async function extractTextFromPDF(pdfBuffer) {
  try {
    const data = await pdf(pdfBuffer, {
      max: 500 // Limit pages to prevent memory issues
    });
    
    console.log(`Text extracted successfully (${data.text.length} characters, ${data.numpages} pages)`);
    return data.text;
  } catch (error) {
    console.error('Error parsing PDF:', error);
    throw error;
...
async function extractAndSaveText(record) {
  const startTime = Date.now();
  
  try {
    console.log(`\n=== Processing: ${record._id} ===`);
    console.log(`Legal Name: ${record.Legal_Name || 'Unknown'}`);
    console.log(`FEI Number: ${record.FEI_Number || 'Unknown'}`);
    console.log(`PDF URL: ${record.Download}`);
    ...
    await FDA483.updateOne(
      ...
      { 
        $set: { 
          rawText: rawText,
          textExtractedDate: new Date(),
          textExtractionError: null
        },
        $inc: { textExtractionAttempts: 1 }
      }
    );
    
    const processingTime = Date.now() - startTime;
    console.log(`✓ Success! Text length: ${rawText.length} characters`);
    ...
    console.error(`✗ Error processing ${record._id}:`, error.message);
...
async function processAllRecords(options = {}) {
  const {
    batchSize = 5,
    maxRecords = null,
  ...
  console.log(`Skip previously failed: ${skipFailed}`);
  ...
  try {
    const query = { rawText: { $exists: false } };
    if (skipFailed) {
      query.textExtractionAttempts = { $lt: 3 };
    }
    
    const totalCount = await FDA483.countDocuments(query);
    console.log(`\nFound ${totalCount} records without extracted text`);
    
    if (totalCount === 0) {
      console.log('All records already have extracted text!');
      return results;
    }
    
    const limit = maxRecords ? Math.min(maxRecords, totalCount) : totalCount;
    let processed = 0;
    
    while (processed < limit) {
      const currentBatchSize = Math.min(batchSize, limit - processed);
      
      const records = await FDA483.find(query)
        .limit(currentBatchSize)
        .lean();
      
      if (records.length === 0) break;
      
      console.log(`\n--- Batch: ${processed + 1} to ${processed + records.length} of ${limit} ---`);
      
      for (const record of records) {
        try {
          const result = await extractAndSaveText(record);
        ...
        if (processed % 10 === 0 || processed === limit) {
          ...
          const remaining = (limit - processed) * avgTime;
          console.log(`\nProgress: ${processed}/${limit} (${Math.round(processed/limit*100)}%)`);
          console.log(`Estimated time remaining: ${Math.round(remaining/1000)}s`);
        }
      }
  ...
  console.log(`Average time per record: ${Math.round(totalTime / results.processed)}ms`);
  
  if (results.errors.length > 0) {
    console.log('\nErrors:');
    results.errors.slice(0, 5).forEach(err => {
      console.log(`  - ${err.recordId || 'General'}: ${err.error}`);
    });
...
app.get('/stats', async (req, res) => {
  try {
    const stats = {
      total: await FDA483.countDocuments(),
      withText: await FDA483.countDocuments({ rawText: { $exists: true } }),
      withoutText: await FDA483.countDocuments({ rawText: { $exists: false } }),
      failed: await FDA483.countDocuments({ textExtractionError: { $exists: true } }),
      failedMultipleTimes: await FDA483.countDocuments({ textExtractionAttempts: { $gte: 3 } })
    };
    
    stats.percentComplete = Math.round((stats.withText / stats.total) * 100);
...
// Extract text for single record
app.post('/extract/:recordId', async (req, res) => {
  try {
    const record = await FDA483.findById(req.params.recordId).lean();
    if (!record) {
      return res.status(404).json({ error: 'Record not found' });
    }
...
// Extract text for all records
app.post('/extract-all', async (req, res) => {
  try {
    const options = {
      batchSize: req.body.batchSize || 5,
      maxRecords: req.body.maxRecords || null,
      continueOnError: req.body.continueOnError !== false,
      skipFailed: req.body.skipFailed !== false
    };
    
    if (req.body.background) {
      res.json({ message: 'Text extraction started in background' });
      processAllRecords(options).catch(console.error);
    } else {
      const results = await processAllRecords(options);
      res.json(results);
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// Get sample of extracted text
app.get('/sample/:recordId', async (req, res) => {
  try {
    const record = await FDA483.findById(req.params.recordId, 'rawText Legal_Name').lean();
    if (!record) {
      return res.status(404).json({ error: 'Record not found' });
    }
    
    if (!record.rawText) {
      return res.status(404).json({ error: 'No extracted text found for this record' });
    }
    
    res.json({
      recordId: record._id,
      legalName: record.Legal_Name,
      textLength: record.rawText.length,
      sample: record.rawText.substring(0, 1000) + '...'
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
async function runCLI() {
  ...
  console.log('FDA 483 Text Extractor - CLI Mode');
  console.log('=================================');
  console.log(`Batch size: ${options.batchSize}`);
  console.log(`Max records: ${options.maxRecords || 'all'}`);
  console.log(`Skip failed: ${options.skipFailed}\n`);

  const stats = await getStats();
  console.log(`Total records: ${stats.total}`);
  console.log(`Already have text: ${stats.withText}`);
  console.log(`Need text extraction: ${stats.withoutText}\n`);

  if (!options.maxRecords && stats.withoutText > 50) {
    console.log('This will extract text from many records.');
...
function printHelp() {
  console.log(`
FDA 483 Text Extractor

Usage: 
  node script.js [options]              # Run CLI extraction
  node script.js --server               # Start API server

CLI Options:
  -b, --batch-size <n>     Process n records at a time (default: 5)
  -m, --max <n>           Maximum records to process
  --include-failed        Include records that failed before
  --stop-on-error         Stop processing on first error
  --stats                 Show statistics only
  -h, --help             Show this help

Server Mode:
  node script.js --server   Starts the API server on port ${PORT}

Examples:
  node script.js                        # Extract all texts
  node script.js -b 10 -m 100          # Process 100 records, 10 at a time
  node script.js --stats               # Show statistics
  node script.js --server              # Start API server
`);
}
...
async function showStats() {
  ...
  console.log('\n=== DATABASE STATISTICS ===');
  console.log(`Total records: ${stats.total}`);
  console.log(`With extracted text: ${stats.withText}`);
  console.log(`Without text: ${stats.withoutText}`);
...
async function main() {
  try {
    ...
    if (process.argv.includes('--server')) {
      ...
      app.listen(PORT, () => {
        console.log(`FDA 483 Text Extractor Server`);
        console.log(`==============================`);
        console.log(`Running on port ${PORT}`);
        console.log(`\nEndpoints:`);
        console.log(`  GET  /health                - Server health`);
        console.log(`  GET  /stats                 - Extraction statistics`);
        console.log(`  POST /extract/:recordId     - Extract text for one record`);
        console.log(`  POST /extract-all           - Extract text for all records`);
        console.log(`  GET  /sample/:recordId      - View text sample`);
        console.log(`\nTo extract all texts via API:`);
        console.log(`  curl -X POST http://localhost:${PORT}/extract-all`);
      });
    } else {
      // Run CLI mode
      await runCLI();
...
// Update stats percentage before returning
async function getStats() {
  const stats = {
    total: await FDA483.countDocuments(),
    withText: await FDA483.countDocuments({ rawText: { $exists: true } }),
  ...
  stats.percentComplete = stats.total > 0 ? Math.round((stats.withText / stats.total) * 100) : 0;
  return stats;
}
